 ...
 * @warning     L'accesso alla tabella non è thread safe, per questo si raccomanda di invocare la funzione dopo aver 
 *              invocato conc_hasht_lock() con gli stessi parametri.
 *
 ...
 * @brief       Consente di stabilire se key è presente nella tabella accedendo al segmento della tabella in mutua esclusione.
 * @warning     Se è stata precedentemente invocata conc_hasht_lock() con gli stessi parametri la lock acquisita verrà 
 *              rilasciata all'uscita della funzione.
 *
 * @param cht   Oggetto che rappresenta la tabella hash thread safe
 * @param key   Chiave che si intende ricercare
 *
 ...
 * @brief       Acquisce la lock sul segmento a cui appartiene key (o apparterrebbe se non presente), permette perciò di 
 *              accedere successivamente a quel segmento in mutua esclusione.
 *
 * @param cht   Oggetto che rappresenta la tabella hash thread safe
 ...
 *              pthread_mutex_lock() in tal caso errno viene settato con il valore che pthread_mutex_lock() ritorna.
 */ 
 ...
 *              EINVAL se cht è @c NULL o key è @c NULL 
 * @note        Può fallire e settare errno se si verificano gli errori specificati da 
 *              pthread_mutex_unlock() in tal caso errno viene settato con il valore che pthread_mutex_unlock() ritorna.
 */
 ...
 * @warning     L'accesso alla tabella non è thread safe, per questo si raccomanda di invocare la funzione dopo aver 
 *              invocato conc_hasht_lock() con cht e key come parametri.
 *
 * @param cht   Oggetto che rappresenta la tabella hash thread safe
 ...
 * @brief       Inserisce la chiave key e il valore value nella tabella se key non è già presente accedendo al segmento 
 *              della tabella in mutua esclusione.
 * @warning     Se è stata precedentemente invocata conc_hasht_lock() con gli stessi parametri la lock acquisita verrà 
 *              rilasciata all'uscita della funzione.
 *
 * @param cht   Oggetto che rappresenta la tabella hash thread safe
 ...
 *              In caso di fallimento errno può assumere i seguenti valori:
 *              EINVAL se cht è @c NULL o key è @c NULL 
 * @note        Può fallire e settare errno se si verificano gli errori specificati da hasht_insert(), pthread_mutex_lock e 
 *              pthread_mutex_unlock. Nel caso di fallimento di pthread_mutex_lock() e pthread_mutex_unlock() errno viene 
 *              settato con i valori che tali funzioni ritornano.
 */
 ...
 * @brief               Elimina dalla tabella la entry la cui chiave è uguale a key accedendo al segmento della tabella in 
 *                      mutua esclusione. Se free_key è diversa da @c NULL utilizza tale funzione per deallocare la chiave 
 *                      uguale a key. Se free_value è diverso da @c NULL utilizza tale funzione per deallocare il valore 
 *                      associato a key.
 * @warning             Se è stata precedentemente invocata conc_hasht_lock() con gli stessi parametri la lock acquisita 
 *                      verrà rilasciata all'uscita della funzione.
 * 
 * @param cht           Oggetto che rappresenta la tabella hash thread safe
 * @param key           Chiave che si intende eliminare
 * @param free_key      Funzione per deallocare la chiave uguale a key nella tabella
 ...
 *                      Nel caso di fallimento di pthread_mutex_lock() e pthread_mutex_unlock() errno viene settato con i 
 *                      valori che tali funzioni ritornano.
 */
 ...
 * @brief           Elimina dalla tabella la entry la cui chiave è uguale a key restituendo il riferimento al valore 
 ...
 * @brief           Elimina dalla tabella la entry la cui chiave è uguale a key accedendo al segmento della tabella in mutua 
 *                  esclusione e restituendo il riferimento al valore associato alla chiave.
 *                  Se free_key è diversa da @c NULL utilizza tale funzione per deallocare la chiave uguale a key.
 * @warning         Se è stata precedentemente invocata conc_hasht_lock() con gli stessi parametri la lock acquisita verrà 
 *                  rilasciata all'uscita della funzione.
 * 
 ...
 *                  Nel caso di fallimento di pthread_mutex_lock() e pthread_mutex_unlock() errno viene settato con i valori 
 *                  che tali funzioni ritornano.
 */
