 ...
 *                se non ci sono thread liberi si cerca di inserire il task in coda. Se non c'e' posto nella coda interna 
 *                allora la chiamata fallisce. 
 * 
 ...
 * @return        0 in caso di successo, 1 se non ci sono thread disponibili e/o la coda è piena, -1 in caso di fallimento 
 *                ed errno settato ad indicare l'errore.
 *                In caso di fallimento errno può assumere i seguenti valori:
 *                EINVAL se pool è @c NULL
 * @note          Può fallire e settare errno se si verificano gli errori specificati da pthread_mutex_lock(), 
 *                pthread_mutex_unlock() e pthread_cond_signal().
 *                Nel caso di fallimento di pthread_mutex_lock(), pthread_mutex_unlock() e pthread_cond_signal() errno viene 
 *                settato con i valori che tali funzioni ritornano.
 */
